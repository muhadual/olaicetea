...
window.DOMElementHandler = class extends self.DOMHandler {
    ...
    _OnCreate(a) {
        ...
        this._autoAttach && (a = this.GetRuntimeInterface().GetAvailableHTMLIndex(e),
    ...
    _OnHTMLLayersChanged() {
        if (this._autoAttach)
            for (const b of this._elementMap.values()) {
                const c = this.GetRuntimeInterface().GetAvailableHTMLIndex(b.GetWantHTMLIndex());
                var a = b.GetActualHTMLIndex();
                -1 !== c && -1 !== a && c !== a && (a = b.GetElement(),
                a.remove(),
...
async function CheckSupportsWorkerMode() {
    ...
    try {
        ...
        return console.warn("Error feature detecting worker mode: ", a),
        !1
    }
}
...
window.RuntimeInterface = class a {
    constructor(b) {
        ...
        this.IsAnyWebView2Wrapper() ? self.chrome.webview.addEventListener("message", c => this._OnWrapperMessage(c.data, c.additionalObjects)) : "macos-wkwebview" === this._exportType && (self.C3WrapperOnMessage = c => this._OnWrapperMessage(c));
        ...
        this.AddRuntimeComponentMessageHandler("canvas", "update-size", c => this._OnUpdateCanvasSize(c));
    ...
    _GetHTMLWrapElement(b) {
        return this.GetHTMLWrapElement(b)
    }
    GetRuntimeBaseURL() {
    ...
    async _Init(b) {
        ...
        "macos-wkwebview" === this._exportType ? this._SendWrapperMessage({
            type: "ready"
        }) : this.IsAnyWebView2Wrapper() && (this._SetupWebView2Polyfills(),
        this._wrapperComponentIds = (await this._InitWrapper()).registeredComponentIds);
        ...
        if ("nwjs" === this._exportType && self.nw && self.nw.App.manifest["c3-steam-mode"]) {
            ...
            this._AddRAFCallback( () => {
                d++;
                document.documentElement.style.opacity = 0 === d % 2 ? "1" : "0.999"
            }
            )
        }
    ...
    GetCssDisplayMode() {
        ...
        return (new Set(["cordova", "nwjs", "macos-wkwebview"])).has(b) ? "standalone" : window.matchMedia("(display-mode: fullscreen)").matches ? "fullscreen" : window.matchMedia("(display-mode: standalone)").matches ? "standalone" : window.matchMedia("(display-mode: minimal-ui)").matches ? "minimal-ui" : navigator.standalone ? "standalone" : "browser"
    }
        ...
        if ("cordova" === this._exportType)
            for (let k = 0, m = b.projectScripts.length; k < m; ++k) {
                const n = b.projectScripts[k]
                  , p = n[0];
                if (p === b.mainProjectScript || "scriptsInEvents.js" === p || p.endsWith("/scriptsInEvents.js"))
        ...
        for (let[g,l] of h)
            ...
            g === f)
                try {
                    ...
                    await AddScript(l),
                    "preview" !== this._exportType || e[g] || this._ReportProjectMainScriptError(g, "main script did not run to completion")
                } catch (k) {
    ...
    _OnUpdateCanvasSize(b) {
        if (!this.IsExportingToVideo()) {
            ...
            for (const {canvas: h, htmlWrap: g} of this._canvasLayers)
                ...
                g.style.marginTop = f,
                this._isFirstSizeUpdate && (h.style.display = "",
                g.style.display = "");
    ...
    _OnSetHTMLLayerCount(b) {
        ...
        if (c < this._canvasLayers.length)
            ...
            for (let k = 0, m = c - this._canvasLayers.length; k < m; ++k)
                ...
                this._useWorker ? (d = c.transferControlToOffscreen(),
                g.push(d),
                l.push(d)) : g.push(c),
    ...
    _OnCleanUpHTMLLayers() {
        for (const b of this._pendingRemoveElements)
            b.remove();
        this._pendingRemoveElements.length = 0
    }
    _UpdateHTMLElementsZOrder() {
        this._pendingUpdateHTMLZOrder || (this._pendingUpdateHTMLZOrder = !0,
    ...
    _DoUpdateHTMLElementsZOrder() {
        ...
        for (; e < f; ++e) {
            const h = b[e];
            h.GetActualHTMLIndex() !== c && (this._DoUpdateHTMLElementsZOrderOnHTMLLayer(c, b.slice(d, e)),
    ...
    _OnWrapperMessage(b, c) {
        if ("entered-fullscreen" === b)
            ...
            const d = b.type;
            "directory-handles" === d ? this._directoryHandles = c : "wrapper-init-response" === d ? (this._wrapperInitResolve(b),
            this._wrapperInitResolve = null) : "extension-message" === d ? this.PostToRuntimeComponent("runtime", "wrapper-extension-message", b) : console.warn("Unknown wrapper message: ", b)
        } else
    ...
    _BinaryStringToBlob(b, c) {
        var d = b.length;
        let e = d >> 2, f = new Uint8Array(d), h = new Uint32Array(f.buffer,0,e), g, l;
        for (l = g = 0; g < e; ++g)
            h[g] = b.charCodeAt(l++) | b.charCodeAt(l++) << 8 | b.charCodeAt(l++) << 16 | b.charCodeAt(l++) << 24;
        for (d &= 3; d--; )
            f[l] = b.charCodeAt(l),
            ++l;
        return new Blob([f],{
...
RuntimeInterface$jscomp$1.AddDOMHandlerClass(class extends self.DOMHandler {
    constructor(a) {
        ...
        a.AddRuntimeComponentMessageHandler("runtime", "raster-svg-image", c => this._OnRasterSvgImage(c));
        ...
        a.AddRuntimeComponentMessageHandler("runtime", "export-to-video-progress", c => this._OnExportVideoProgress(c));
        ...
        if (a.IsiOSWebView()) {
            ...
            window.visualViewport.addEventListener("resize", () => {
                ...
                d > c && (document.scrollingElement.scrollTop = 0,
    ...
    _OnAndroidVirtualKeyboardChange(a, b) {
        ...
        if (0 < b) {
            ...
            a > b && (a = b),
            0 > a && (a = 0),
            0 < a && (document.body.style.position = "absolute",
    ...
    _OnVisibilityChange(a) {
        this._pageVisibilityIsHidden !== a && ((this._pageVisibilityIsHidden = a) ? this._iRuntime._CancelAnimationFrame() : this._iRuntime._RequestAnimationFrame(),
    ...
    _OnKeyEvent(a, b) {
        ...
        "nwjs" === this._iRuntime.GetExportType() && "u" === b.key && (b.ctrlKey || b.metaKey) && b.preventDefault();
        if (!this._isExportToVideo) {
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    constructor(a) {
        ...
        this._beforeunload_handler = b => b.preventDefault();
        document.addEventListener("backbutton", () => this._OnCordovaBackButton())
    }
    Attach() {
        deferredInstallPromptEvent ? this._OnBeforeInstallPrompt() : browserDomHandler = this;
    ...
    async _OnRequestInstall() {
        ...
        try {
            return deferredInstallPromptEvent.prompt(),
            {
                result: (await deferredInstallPromptEvent.userChoice).outcome
            }
        } catch (a) {
            return console.error("[Construct] Requesting install failed: ", a),
            {
                result: "failed"
            }
        }
    ...
    _OnNavigate(a) {
        ...
        if ("back" === b)
            ...
            const c = a.target;
            a = a.exportType;
            self.cordova && self.cordova.InAppBrowser ? self.cordova.InAppBrowser.open(b, "_system") : "preview" === a || this._iRuntime.IsAnyWebView2Wrapper() ? window.open(b, "_blank") : this._isConstructArcade || (2 === c ? window.top.location = b : 1 === c ? window.parent.location = b : window.location = b)
        } else
    ...
    _OnRequestFullscreen(a) {
        if (this._iRuntime.IsAnyWebView2Wrapper() || "macos-wkwebview" === this._exportType)
    ...
    _OnSetWarnOnClose(a) {
        a.enabled ? window.addEventListener("beforeunload", this._beforeunload_handler) : window.removeEventListener("beforeunload", this._beforeunload_handler)
    }
}
);
"use strict";
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    async _Init() {
        ...
        try {
            ...
            a = !1,
            console.warn("[Construct] Error checking storage persisted state: ", b)
        }
        return {
            isPersistent: a
        }
...
self.AudioDOMHandler = class extends self.DOMHandler {
    ...
    async _CreateAudioContext(a) {
        a.usePlayMusicAsSoundWorkaround && (this._playMusicAsSound = !0);
    ...
    *audioInstancesByEffectTag(a) {
    ...
    async _Play(a) {
        ...
        try {
            ...
            k ? (this._lastAudioInstance.SetPannerEnabled(!0),
            this._lastAudioInstance.SetPan(k.x, k.y, k.z, k.angle, k.innerAngle, k.outerAngle, k.outerGain),
            k.hasOwnProperty("uid") && this._lastAudioInstance.SetUID(k.uid)) : "number" === typeof m && 0 !== m ? (this._lastAudioInstance.SetStereoPannerEnabled(!0),
...
self.C3AudioInstance = class {
    ...
    SetPan(a, b, c, d, e, f, h) {
        ...
        this._pannerConeParams[2] !== h && (this._pannerConeParams[2] = h,
